    ...
    <div class="bg-bb-charcoal p-10 max-w-screen-md rounded-lg dark:border dark:border-bb-light md:mx-auto mx-5 relative">
      <client-only>
        ...
        <form @submit.prevent="sendComment" novalidate>
          ...
          <UiButton type="submit" classes="btn-dark" :loading="isLoading" :disabled="isDisabled">abschicken</UiButton>
        </form>
      </client-only>
      <button @click="$emit('close', true)" class="top-4 right-4 absolute text-bb-lighter"><IconClose fill="currentColor" /></button>
    </div>
  </div>
</template>
