<template>
  <div class="max-w-screen-md mx-auto flex flex-col justify-between py-26">
    ...
    <template v-for="(post, index) in posts?.data" :key="post.id">
      ...
      <PostHtml v-if="post.type === 'html'" :post="post" @comment-added="onCommentAdded" />
    ...
    <CommonPagination v-if="posts && posts.meta.last_page > 1" :offset="3" :pagination="posts.meta" @paginate="changePage" label="Seitennavigation Beiträge (unten)" class="my-12 relative flex justify-center" />
    </template>
  </div>
</template>
...
import type { IStory, IPost, IComment } from "@/types";
...
const storyDescription = computed(() => sanitizeHtml(story.value?.description));
...
function onCommentAdded(postId: number, comment: IComment) {
  const target = posts.value?.data.find((p) => p.id === postId);
  if (target) {
    target.comments = target.comments ? [...target.comments, comment] : [comment];
  }
}
...
const orderToggleLabel = computed(() =>
  getOrder() === "asc"
    ? "Sortierung: aufsteigend. Klicken für absteigend."
    : "Sortierung: absteigend. Klicken für aufsteigend."
);
