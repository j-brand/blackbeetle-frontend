<template>
  <div id="poem" class="h-full flex justify-center mb-12">
    <div class="poem inline-block ml-8 md:ml-24">
...
if (import.meta.client) {
  ...
  function animateParticules(x: number, y: number) {
    ...
    tl.add(particules, {
      x: ((_el: unknown, i: number) => particules[i]!.endPos.x) as unknown as number,
      y: ((_el: unknown, i: number) => particules[i]!.endPos.y) as unknown as number,
      radius: 0.1,
  ...
  onMounted(() => {
    ...
    if (logoWrapper) {
      logoWrapper.insertAdjacentHTML(
        "beforeend",
        `<div id="logo-present" class="z-10"><svg class="mt-6 p-3 text-fg" style="width:85px;height:85px" viewBox="-3.5 -3 30 30">
